...
if ! grep -q '^SECRET_KEY=' .env || [[ -z "$(grep '^SECRET_KEY=' .env | tail -n1 | cut -d'=' -f2-)" ]]; then
  cat >&2 <<'EOF'
[deploy] SECURITY ERROR: SECRET_KEY must be set explicitly in .env before deployment.
[deploy] Generate one manually, for example:
[deploy]   python3 -c "import secrets; print(secrets.token_hex(32))"
EOF
  exit 1
fi
